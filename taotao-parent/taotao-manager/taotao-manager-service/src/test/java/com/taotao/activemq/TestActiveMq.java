...
//		ConnectionFactory connectionFactory = new ActiveMQConnectionFactory("tcp://192.168.156.70:61616");
...
//		//1.创建一个连接工厂对象ConnectionFactory对象。需要指定mq服务的ip及端口号。注意参数brokerURL的开头是
//		//tcp://而不是我们通常的http://，端口是61616而不是我们访问activemq后台管理页面所使用的8161
//		ActiveMQConnectionFactory connectionFactory = new ActiveMQConnectionFactory("tcp://192.168.156.70:61616");
...
////		textMessage.setText("hello,activemq!!!");
//		//第二种方式：
//		TextMessage textMessage = session.createTextMessage("hello,activemq topic6");
//		//11.发送消息
//		producer.send(textMessage);
//		//12.关闭资源
//		producer.close();
...
//		connection.setClientID("consumer3");
//		//5.开启连接。调用Connection对象的start方法
//		connection.start();
//		//6.使用Connection对象创建一个Session对象
//		//第一个参数是是否开启事务，一般不使用分布式事务，因为它特别消耗性能，而且顾客体验特别差，现在互联网的
...
//		MessageConsumer consumer = session.createDurableSubscriber(topic, "consumer3");
